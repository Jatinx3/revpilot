...
create table if not exists public.reservations_hackathon (
  ...
  -- NOTE (deliberate deviation from the brief's schema): source bookings carry
  -- granular rate codes (e.g. OCHEARLY, EXPBARB, BARCBB) that are NOT all present
  -- in the 8-row rate_plan_lookup reference, while /verify still expects exactly 8
  -- reference rows. The brief's FK to rate_plan_lookup therefore cannot hold against
  -- the real data, so it is intentionally NOT enforced here — we preserve every
  -- booking row rather than drop unmapped ones or pollute the reference table.
  -- See ARCHITECTURE.md / etl/SITE_NOTES.md.
